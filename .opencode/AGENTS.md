...
| --- | --- | --- | --- |
| Brooks | Frederick P. Brooks Jr. | Architecture and orchestration | `flat/` |
| Jobs | Steve Jobs | Intent gate and scope owner | `flat/` |
| Woz | Steve Wozniak | Primary builder | `flat/` |
| Bellard | Fabrice Bellard | Deep diagnostics | `flat/` |
| Carmack | John Carmack | Performance | `flat/` |
| Scout | Utility role | Discovery and recon (ContextScout) | `flat/` |
| Pike | Rob Pike | Interface simplicity | `flat/` |
| Fowler | Martin Fowler | Refactor safety | `flat/` |
| Knuth | Donald Knuth | Data and schema | `flat/` |
| Hightower | Kelsey Hightower | Infra and deployment | `flat/` |

## Product Agents (Non-Team RAM)

| Agent | Persona | Role | Path | Mode |
| --- | --- | --- | --- | --- |
| Bahari | Bahari | Memory curator, guided capture, hygiene, onboarding | `flat/` | Primary |

> Bahari is a product companion, not a Team RAM subagent. She does not report to Brooks and does not participate in the surgical team. She is a peer interface shipped alongside Team RAM.
...
| Bahari | `agent-bahari`, `allura-memory-skill` | `mcp-docker` | Product companion; uses user's group_id, never `allura-system`. |
