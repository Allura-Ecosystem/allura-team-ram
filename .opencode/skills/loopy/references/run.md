...
## Allura addition — Ralph delegation

loopy Run does not spin up a second execution engine. It delegates to Ralph for
bounded execution:
...
The loop file should be written to `ralph/goals/[loop-name].loop.md` or a
temporary prompt file. Ralph runs it with the supplied `--max-iterations` and
returns. loopy then formats the receipt.
