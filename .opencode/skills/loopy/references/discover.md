...
Before discovery, hydrate from Allura Brain:

```
allura-brain__memory_search({
  query: "repeated work patterns agent outcomes",
  group_id: "allura-system",
  limit: 10
})
```

Brain holds SONA trajectories and prior agent outcomes — this is evidence loopy
can use alongside codebase recon. Scout does the codebase recon; loopy
...
2. **Allura addition:** also search Brain for a saved project loop that fits.
   Present a Brain-sourced match as the project's own loop.
3. If several candidates are similarly strong or differ materially in
...
4. Derive the trigger, fresh observation, bounded action, reproducible
...
5. Run the mandatory crafted-loop preflight in `SKILL.md`. Repair material
   weaknesses before delivery without expanding authority or inventing missing
   details.
6. Return the compact evidence and the loop using the standard delivery format
