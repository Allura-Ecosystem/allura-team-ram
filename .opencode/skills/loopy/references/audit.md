...
In the Team RAM harness, loop audit is one of three orthogonal review gates:

- **Pike** reviews interface/surface area (fewer interfaces, stronger contracts)
- **Fowler** reviews maintainability (lint, typecheck, line count, refactor safety)
- **loopy (Loop Doctor)** reviews loop design (weak checks, unsafe authority, unclear stops)

Do not collapse these into one. Each gate sees a different failure mode.
